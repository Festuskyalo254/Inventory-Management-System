...
try:  # !Connect to your MySQL database!
    project_db = mysql.connector.connect(
        host="localhost",  # Add your database`s info to open.
        user="root",
        password="1234#",
        port="3306",
...
class LoginWindow:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title(" Larrium Office Inventory Management System")
        self.root.geometry("800x500")
        root.configure(bg="#8AA6A3")
        self.setup_ui()
    ...
    def setup_ui(self):
        ...
        # Configure the style
        style.configure("Username.TLabel", font=("Goudy old style", 20, "bold"), foreground="white", background="#333")
        style.configure("Username.TEntry", font=("Goudy old style", 25,"bold"), foreground="white", background="#127369",
                        fieldbackground="#444", highlightthickness=0)
        style.configure("Password.TLabel", font=("Goudy old style", 20, "bold"), foreground="white", background="#333")
        style.configure("Password.TEntry", font=("Goudy old style", 25,"bold"), foreground="white", background="#127369",
                        fieldbackground="#444", show="*", highlightthickness=0)
        style.configure("Login.TButton", font=("Goudy old style", 20, "bold"), foreground="white", background="#333")

#username &password labels
        self.username_icon = tk.PhotoImage("username.png")
        self.username_label = ttk.Label(self.root, text="Username:", image=self.username_icon,style="Username.TLabel", compound=tk.LEFT,font=("Goudy old style", 20, "bold"),border=0)
        self.username_label.pack(pady=30)
        self.username_entry = ttk.Entry(self.root,style="Username.TEntry",width=20,font=("Goudy old style", 20, "bold"))
        self.username_entry.pack(pady=5)

        self.password_label = ttk.Label(self.root, text="Password:",style="Password.TLabel",font=("Goudy old style", 20, "bold"))
        self.password_label.pack(pady=30)
        self.password_entry = ttk.Entry(self.root, show="*",style="Password.TEntry",width=20,font=("Goudy old style",20,"bold"))
        self.password_entry.pack(pady=5)

        self.login_button = ttk.Button(self.root, text="Login", command=self.login,style="Login.TButton")
...
class MyApp:
    ...
    def setup_ui(self):
        ...
        self.notebook.add(self.orders_page, text=" Larrium Orders", image=self.icon_buy, compound=tk.TOP)
        self.notebook.add(self.sales_page, text=" Larrium Sales", image=self.icon_sell, compound=tk.TOP)
        self.notebook.add(self.product_page, text=" Larrium Products", image=self.icon_inventory, compound=tk.TOP)
        self.notebook.add(self.suppliers_page, text=" Larrium Suppliers", image=self.icon_buying, compound=tk.TOP)
        ...
        ttk.Button(self.orders_page, text="Add Order", image=self.icon_add, compound=tk.LEFT,
                   command=self.add_order).pack(pady=10)
        ...
        "OrderID", "OrderDate", "SupplierID", "ProductID", "UserID", "OrderQuantity", "UnitCost")
        ...
        ttk.Button(self.sales_page, text="Record Sale", image=self.icon_add, compound=tk.LEFT,
                   command=self.record_sale).pack(pady=10)

        self.sales_tree = ttk.Treeview(self.sales_page)
        self.sales_tree["columns"] = (
        "SaleID", "ProductID", "SaleQuantity", "ProductName", "UnitPrice", "SupplierName", "UserID", "SalesDate")
        ...
        ttk.Button(self.sales_page, text='Refresh Sales', image=self.icon_refresh, compound=tk.TOP,
                   command=self.show_sales_data).pack(pady=10)
        ...
        ttk.Button(self.refresh_buttons_frame, text='Show Products', image=self.icon_search, compound=tk.LEFT,
                   command=self.show_filtered_data).pack(side=tk.LEFT, padx=10)

        ttk.Button(self.refresh_buttons_frame, text='All Products', image=self.icon_refresh2, compound=tk.LEFT,
                   command=self.show_products_data).pack(side=tk.LEFT, padx=10)
        ...
        ttk.Button(self.product_page, text="Add New Product", image=self.icon_add, compound=tk.LEFT,
                   command=self.add_new_product).pack(pady=10)
        ...
        ttk.Button(self.suppliers_page, text="Refresh Suppliers", image=self.icon_refresh, compound=tk.TOP,
                   command=self.show_suppliers_data).pack(pady=10)
        ...
        ttk.Button(self.suppliers_page, text="Add New Supplier", image=self.icon_add, compound=tk.LEFT,
                   command=self.add_new_supplier).pack(pady=10)
